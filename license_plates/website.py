        ...
        if mesg is not None:
            ...
            if matches is not None:
                ...
                if match_type == "substring match":
                    ...
                    st.write(
                        """This plate closely resembles a word or phrase that 
...
with tab2:
    ...
    uploaded_lics = st.file_uploader(
        "Upload csv of License Plates", type="csv")
    ...
    if uploaded_lics is not None:
        ...
        if input_df.empty:
          ...
          # if not empty, check for correct format (we can add more if needed)
                ...
                # 2. check for any matches with unscrambler + evaluate function
                    ...
                    if matches is not None:
                        ...
                        if match_type == "substring match":
                            ...
                            exact_msg = f"""The detected word is {matches[1]}.
                            The detection method is exact match."""

                        else:
                            reason = """This plate closely resembles a word or
                            phrase that may be considered inappropriate."""
                            exact_msg = f"""Plate closely resembles the restricted word
                            '{matches[1]}' (similarity score: {matches[2]}%)."""
            ...
            data = {
                'plate': input_df['plate'], 'approved?': decisions,
                'reason': reasons, 'notes': notes}
            ...
            st.info("""Note: The decisions shown are generated by an automated
            screening tool and are intended to assist DMV staff. 
