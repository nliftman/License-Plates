    ...
    if uploaded_lics is not None:
        ...
        if input_df.empty:
          ...
          # if not empty, check for correct format (we can add more if needed)
                ...
                # 2. check for any matches with unscrambler + evaluate function
                    ...
                    if matches is not None:
                        ...
                        if match_type == "substring match":
                            ...
                            exact_msg = f"""The detected word is
                            {matches[1]}. The detection method is exact match."""

                        else:
                            reason = """This plate closely resembles a word or phrase
            ...
            st.info("""Note: The decisions shown are generated by an automated screening
            tool and are intended to assist DMV staff.
